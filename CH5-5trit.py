...
#######                                          #########
#######                                          #########
fileEnergy = open("5trit_energies.txt", "a+")
fileXYZ = open("5trit_ancestors.xyz", "a+")
#fileHistData = open("DCHhistdata.txt", "a+")
#fileBinEdges = open("DCHbinedges.txt", "a+")
timestep = 0
energies = []
population = []
ancestorPile = []
...
for c in range(0,cycles):    
	...
	displace = np.random.normal(0,sigma4,whereUat.shape)
	...
	if timestep%500 == 50 and timestep >= 7500:
		for i in whereUfrom:
